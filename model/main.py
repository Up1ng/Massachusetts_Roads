...
class ResNet18UNet(nn.Module):
    def __init__(self):
        ...
        self.final_up = nn.Sequential(nn.ConvTranspose2d(64, 32, 2, 2), nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        x = self.bottleneck(self.encoder4(skip3))
...
def find_best_threshold(model, loader, device):
    candidates = [round(value, 2) for value in np.arange(0.3, 0.71, 0.05)]
...
def main():
    ...
    best_dice, best_model = run_stage(
        model,
        train_loader,
        val_loader,
        criterion,
    ...
    val_metrics = evaluate(model, val_loader, criterion, device, threshold=THRESHOLD)
    print(
        f"val_dice_at_0.50={val_metrics['dice']:.4f} | "
